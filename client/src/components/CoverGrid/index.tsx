...
const randomEmojis = () => {
  ...
  randomEmojis.forEach(() => {
    if (finalEmojis.length < 12) {
      const emoji_string = randomEmojis[Math.floor(Math.random() * randomEmojis.length)]
  ...
  for (let i = 0; i < finalEmojis.length; i++) {
    let temp = emojiIndex.search(finalEmojis[i]) as { colons: string; native: string }[] | null
    if (temp) {
...
const CoverGrid = () => {
  const [emojis, setEmojis] = useState<{ colons: string; native: string }[]>(randomEmojis())
    ...
    <>
      <div className="grid-body">
        <div className="cover">
          ...
          <div className="emoji-grid">
            {emojis.map((emoji, key) => (
              <div className={`emoji-container ${loading ? "disable-emoji" : ""}`} key={key}>
                <motion.button
                  disabled={loading}
                  className={`emoji-button ${emptyGrid ? "empty-grid" : ""} ${loading ? "disabled" : ""}`}
                  data-key={key}
                  onClick={handleOpen}
            ...
            >
              <Button buttonDanger className="remove-emoji" onClick={() => handleChange()}>
                remove emoji
              </Button>
              <NimblePicker
                showPreview={false}
        ...
        <div className="actions">
          <Button disabled={loading} buttonDanger fullWidth onClick={() => handleRemoveAllEmojis()}>
            Remove all emojis
          </Button>
          ...
          <div className="background-color">
            <TwitterPicker color={color} onChange={(e) => handleChangeColor(e)} />
          </div>
        </div>
      </div>
    </>
